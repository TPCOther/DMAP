    ...
    def get_attentions_outputs(self, dataset, batch_size):
        ...
        for batch in eval_dataloader:
            ...
            with torch.no_grad():
                attention = self.encoder.embeddings(inputs)
                for i in range(12):
                    attention_output = self.encoder.encoder.layer[i].attention(attention, attention_mask)[0]
                    attention = self.encoder.encoder.layer[i](attention, attention_mask, output_attentions=True)
                    attention_score = attention[0]
                    attention = attention[0]
                    attention_temp.append(attention_output)
                    layer_attentions.append(attention_score)
                attention_outputs.append(torch.stack(attention_temp, dim=1))
