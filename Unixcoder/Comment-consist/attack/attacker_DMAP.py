...
from utils import select_parents, crossover, map_chromesome, mutate,  _tokenize, \
    get_identifier_posistions_from_code, get_masked_code_by_position, is_valid_variable_name,\
    CodeDataset, is_valid_identifier, get_code_tokens, isUID, remove_comments_and_docstrings, is_valid_identifier, \
...
class ALERT_Attacker():
    def __init__(self, args, model_tgt, tokenizer_tgt, model_mlm, tokenizer_mlm, use_bpe, threshold_pred_score) -> None:
        ...
        self.DMAP = DMAP(1, "../probe/ProbeClassifier.pth", 2, 6, "cuda")
    ...
    def greedy_attack(self, example, substitutes, code):
        ...
        for name_and_score in sorted_list_of_names:
            ...
            all_substitues = self.DMAP.get_sub_by_weight_reverse(all_substitues)
            mix_substitutes[tgt_word] = all_substitues
            ...
            # logits, preds = self.DMAP.verify_logist(self.model_tgt, self.args.eval_batch_size, adjust=False)
                    ...
                    if temp_label != orig_label:
                        ...
                        print("%s SUC! %s => %s (%.5f => %.5f)" % \
                              ('>>', tgt_word, candidate,
                               current_prob,
                               temp_prob[orig_label]), flush=True)
                        return code, prog_length, adv_code, true_label, orig_label, temp_label, is_success, variable_names, names_to_importance_score, nb_changed_var, nb_changed_pos, replaced_words, sucpre_code
                    else:
                        gap = current_prob - temp_prob[temp_label]
                        if gap > 0:
                            golden_prob_decrease_track[substitute_list[index]] = gap
                            sucpre_code = get_example(final_code, tgt_word, substitute_list[index], "java")
