...
def get_code_tokens(code):
    ...
    try:
        for token in tokens:
            code_tokens.append(token.value)
    except:
        pass
    # code_tokens = [token.value for token in tokens]
    return code_tokens
...
def mutate(chromesome, variable_substitue_dict):
    mutate_chromesome = copy.deepcopy(chromesome)
    tgt_index = random.choice(range(len(chromesome)))
    tgt_word = list(chromesome.keys())[tgt_index]
    mutate_chromesome[tgt_word] = random.choice(variable_substitue_dict[tgt_word])

    return mutate_chromesome

def pos_mutate(chromesome):
    mutate_chromesome = copy.deepcopy(chromesome)
    tgt_index_1 = random.choice(range(len(chromesome)))
    tgt_index_2 = random.choice(range(len(chromesome)))
    tgt_key_1 = list(chromesome.keys())[tgt_index_1]
    tgt_key_2 = list(chromesome.keys())[tgt_index_2]
    mutate_chromesome[tgt_key_1], mutate_chromesome[tgt_key_2] = mutate_chromesome[tgt_key_2], mutate_chromesome[tgt_key_1]
...
class CodeDataset(Dataset):
    ...
    def subset(self, size):
        # 限制索引的大小以截取数据集的一部分
        self.order = self.order[:size]

    def __getitem__(self, i):
        idx = self.order[i]
        return torch.tensor(self.examples[idx].input_ids),torch.tensor(self.examples[idx].label)
