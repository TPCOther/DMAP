...
def get_example(code, tgt_word, substitute, lang):
    #正则表达式替换分不清要替换的词是否是目标变量而非其他元素，使用传统处理字符串进行替换
    parser = parsers[lang]
    code = code.replace("\\n", "\n")
    parser = parsers[lang]
    tree = parser[0].parse(bytes(code, 'utf8'))
    root_node = tree.root_node
    tokens_index = tree_to_token_index(root_node)   #每个代码元素的起始和结束位置
    code = code.split('\n') #代码行
    code_tokens = [index_to_code_token(x, code) for x in tokens_index]  #从代码中拆出token
...
def get_example_batch(code, chromesome, lang):
    ...
    tree = parser[0].parse(bytes(code, 'utf8'))
    root_node = tree.root_node
    #tokens_index为一个list,存放了每个代码元素的起始和结束位置，而起始结束位置又由(行，列)组出成
    #tokens_index[index]代表第index个代码元素, tokens_index[index][0]代表第index个代码元素的起始位置，tokens_index[index][0][0]代表第index个代码元素的开始位置行号
    tokens_index = tree_to_token_index(root_node)
    code = code.split('\n')
    code_tokens = [index_to_code_token(x, code) for x in tokens_index]
    replace_pos = {}
    ...
    for line in replace_pos.keys():
        replace_pos[line].sort(key=lambda x: x[3], reverse=True)
        diff = 0
        for index, pos in enumerate(replace_pos[line]):
            code[line] = code[line][:pos[3]] + pos[1] + code[line][pos[4]:]
            diff += pos[2]
