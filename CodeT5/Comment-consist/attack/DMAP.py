...
def get_attention_output(model, dataset, target_layer=5, batch_size=16):
    output_score, bias, input, _ = model.get_attentions_output(dataset, target_layer, batch_size)
    output_scores = output_score
    return output_scores, bias, input, output_score
...
class DMAP:
    ...
    def get_att_order(self, tgt_model, dataset, ori_att, ori_label):
        atts, bias, inputs, _ = get_attention_output(tgt_model, dataset, self.target_layer)
