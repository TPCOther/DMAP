...
from attacker_DMAP import ALERT_Attacker, convert_examples_to_features
...
def main():
    ...
    config.num_layers = 4
    att_model = T5EncoderModel.from_pretrained('Salesforce/codet5-base-multi-sum', config=config)
    att_model.load_state_dict(torch.load("../saved_models/checkpoint-best-f1/encoder.bin"), False)
    att_model.to(args.device)
    ...
    attacker = ALERT_Attacker(args, model, tokenizer, codebert_mlm, tokenizer_mlm, use_bpe=1, threshold_pred_score=0, att_model=att_model)
