...
class Model(nn.Module):   
    ...
    def get_attentions(self, dataset, batch_size, threshold=0.5):
        '''Given a dataset, return probabilities and labels.'''
        eval_sampler = SequentialSampler(dataset)
        eval_dataloader = DataLoader(dataset, sampler=eval_sampler, batch_size=batch_size,num_workers=0,pin_memory=False)
        ...
        for batch in eval_dataloader:
            ...
            label)=[x.to("cuda")  for x in batch]
            with torch.no_grad():
                attention = self.forward(inputs_ids_1,position_idx_1,attn_mask_1,inputs_ids_2,position_idx_2,attn_mask_2, output_attentions=True)
