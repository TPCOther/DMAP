...
class Model(nn.Module):
    ...
    def get_attentions_output(self, dataset, layer, batch_size):
        ...
        for batch in eval_dataloader:
            ...
            with torch.no_grad():
                attention = self.encoder.model.encoder.embed_tokens(inputs) * self.encoder.model.encoder.embed_scale
                embed_pos = self.encoder.model.encoder.embed_positions(inputs)
                attention = self.encoder.model.encoder.layernorm_embedding(attention + embed_pos)
                attention = attention, None
                for i in range(layer):
                    attention = self.encoder.model.encoder.layers[i](attention[0], attention_mask=attention_mask, layer_head_mask=None)
                attention_output = self.encoder.model.encoder.layers[i].self_attn(attention[0], attention_mask=attention_mask, layer_head_mask=None)
        ...
        for batch in eval_dataloader:
            ...
            with torch.no_grad():
                ...
                for i in range(12):
                    ...
                    layer_attentions.append(attention[2])
    ...
    def continue_forward(self, data, position_bias, input_ids, layer, batch_size):
        ...
        for i in range(0, len(data), batch_size):
            ...
            with torch.no_grad():
                ...
                decoder_input_ids = shift_tokens_right(inputs, self.tokenizer.pad_token_id)
                decoder_hidden_state = self.encoder.model.decoder(input_ids=decoder_input_ids, encoder_hidden_states = encoder_hidden_state, encoder_attention_mask=ori_attention_mask)
