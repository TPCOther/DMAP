...
from attacker import WIR_Attacker, convert_examples_to_features
from transformers import (RobertaForMaskedLM, T5Config, T5ForConditionalGeneration, T5EncoderModel, RobertaTokenizer, PLBartConfig, PLBartForConditionalGeneration, PLBartTokenizer)
...
def main():
    ...
    with open(args.csv_store_path, "w") as wf:
        for index, example in enumerate(eval_dataset):
            ...
            code, prog_length, adv_code, true_label, orig_label, temp_label, is_success, variable_names, names_to_importance_score, nb_changed_var, nb_changed_pos, replaced_words, sucpre_code = attacker.wir_random_attack(
                example, code_pair)
