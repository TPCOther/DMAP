...
from DMAP_FFN import DMAP, get_attention_output
...
def get_attention(model, tokenizer, dataset):
    ...
    for batch in eval_dataloader:
        ...
        with torch.no_grad():
            attention = model(input_ids, attention_mask=attention_mask, output_attentions=True).attentions
        attention = tuple(att.detach().cpu() for att in attention)
...
def get_att_score(args, example, code, words_list: list, variable_names: list, att_model, tokenizer, label_list, batch_size=16, max_length=512, model_type='classification'):
    ...
    importance_score = []
    for att in atts[1:]:
        importance_score.append(1 - get_cosine_similarity(orig_att, att[-1]).item())
...
def get_att_order(model, tokenizer, dataset, ori_att):
    ...
    sim_score = []
    for att in atts:
        sim_score.append(get_cosine_similarity(ori_att, att[-1]).item())
    return np.argsort(sim_score)
...
class RNNS_Attacker():
    def __init__(self, args, model_tgt, tokenizer_tgt, model_mlm, tokenizer_mlm, use_bpe, threshold_pred_score) -> None:
        ...
        self.DMAP = DMAP(1, "../probe/ProbeClassifier_encoder_ffn.pth", 2, 3, "cuda")
    ...
    def attack(self, example, code):
        ...
        orig_prob = logits[0]
        orig_label = preds[0]
        orig_att = get_attention_output(self.model_tgt, [example], 3)[0][:, 0, :].squeeze(1)
...
class ALERT_Attacker():
    ...
    def greedy_attack(self, example, code, substituions):
        ...
        # importance_score, replace_token_positions, names_positions_dict = get_att_score(self.args, example, 
        #                                 processed_code,
        #                                 words,
        #                                 variable_names,
        #                                 self.att_model, 
        ...
        # self.model_tgt.query += len(importance_score)+1
