...
from transformers import (RobertaForMaskedLM, RobertaConfig, RobertaForSequenceClassification, 
                          ...
                          ,GPT2Config, GPT2LMHeadModel, GPT2Tokenizer, PLBartConfig, PLBartForConditionalGeneration, PLBartTokenizer)
...
def main():
    ...
    if args.ablation_type == "origin":
        ...
        from attacker_DMAP import ALERT_Attacker
    elif args.ablation_type == "wo":
        print("Ablation type: ", "wo")
        from attacker_DMAP_wo import ALERT_Attacker
    elif args.ablation_type == "wo_att":
        print("Ablation type: ", "wo_att")
        from attacker_DMAP_wo_att import ALERT_Attacker
    elif args.ablation_type == "wo_probe":
        print("Ablation type: ", "wo_probe")
        from attacker_DMAP_wo_probe import ALERT_Attacker
    else:
    ...
    with open(args.csv_store_path, "w") as wf:
        for index, example in enumerate(eval_dataset):
            tmp_save = {"Index":None,"Sucpre Code":None,"Original Code":None,"Adversarial Code":None,"Program Length":None,"Identifier Num":None,"Replaced Identifiers":None,"Query Times":None,"Time Cost":None,"Type":None}
            print("Index: ", index)
            ...
            if len(identifiers) == 0:
                print("No identifiers in this example")
                continue
          
            total_cnt += 1

            code, prog_length, adv_code, true_label, orig_label, temp_label, is_success, variable_names, names_to_importance_score, nb_changed_var, nb_changed_pos, replaced_words, suc_pre_code = attacker.greedy_attack(
                example, code, substitutes)
            attack_type = "Greedy"
            if is_success == -1 and args.use_ga:
                code, prog_length, adv_code, true_label, orig_label, temp_label, is_success, variable_names, names_to_importance_score, nb_changed_var, nb_changed_pos, replaced_words, suc_pre_code = attacker.ga_attack(
            ...
            if is_success == 1:
                ...
                tmp_save["Sucpre Code"] = suc_pre_code
