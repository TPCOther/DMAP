...
def main():
    ...
    config.num_labels = 2
    att_model = RobertaModel.from_pretrained('microsoft/codebert-base', config=config)
    ...
    with open(args.csv_store_path, "w") as wf:
        for index, example in enumerate(eval_dataset):
            ...
            print(identifiers)
            code, prog_length, sucpre_code, adv_code, true_label, orig_label, temp_label, is_success, variable_names, names_to_importance_score, nb_changed_var, nb_changed_pos, replaced_words, mix_substitutes = attacker.greedy_attack(
            ...
            if is_success == 1:
                ...
                tmp_save["substitutes"] = mix_substitutes
